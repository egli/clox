...
** DONE Add string and other types
CLOSED: [2025-11-27 Do 15:40]
...
I would burn and crash, I mean report an error. I wouldn not allow
this implicit type conversion as it leads to all sorts of subtle to
find bugs.

* Global Variables
** DONE Global variable name in the constant table
CLOSED: [2026-01-08 Do 11:02]
The compiler adds a global variable’s name to the constant table as a
string every time an identifier is encountered. It creates a new
...
That, in turn, increases the odds of filling up the constant table and
running out of slots since we allow only 256 constants in a single
chunk.
...
*** Answer
We could just add the variable name and the corresponding index to a
hash table.

The compiler will be slower. The runtime behavior will not be affected
(if you ignore the fact that the constants table will be smaller).

See excercise/21.1 for the implementation.

** DONE Global variable without hash table lookup
CLOSED: [2026-01-08 Do 14:38]
Looking up a global variable by name in a hash table each time it is
used is pretty slow, even with a good hash table. Can you come up with
a more efficient way to store and access global variables without
changing the semantics?

*** Answer
We could keep the global vars in an array in the vm and calculate the
offset at compile time. The index is basically the order of variable
declarations. The compiler keeps a hash map with variable name to
index mapping.

However the compiler only sees one compilation unit (basically a
Chunk), so in the repl this will not work. It should work within a
file.

** DONE Undefined variables
CLOSED: [2026-01-08 Do 14:32]
...
there is no declaration of that global anywhere in the program. Note
that ~useVar()~ is never called either, so even though the variable
isn’t defined, no runtime error will occur because it’s never used
either.
...
*** Answer
Yes, we should, as the variable ~oops~ has not been declared anywhere.
That is most likely a bug. It is much better for the developer if the
compiler reports these kinds of issues. However this probably means we
can no longer have a single pass compiler.

The downside of throwing an error is that
1) it will be inconsistent with the repl
2) multi-pass compiler


**** Other languages
|--------+---------------------------------------------------------|
| Rust   | Strict analysis                                         |
| Python | Allows undefined variables, runtime error when accessed |

* Local Variables
** DONE Replace linear search for locals
CLOSED: [2026-01-20 Di 15:30]
Our simple local array makes it easy to calculate the stack slot of
...
*** Answer
It could be done with a hash map for example, but it's probably not
worth it since we are basically talking about 10 to 20 local
variables.

** TODO Local var used in own initializer
...
*** Answer
- Allow of a has been defined in a surrounding scope
- I think Clojure allows shadowing (let [a 1] (let [a a]))

** TODO Single-assignment variable
...
variable declared using your new keyword should cause a compile error.

** TODO More that 256 local vars
Extend clox to allow more than 256 local variables to be in scope at a
time.

*** Answer
I guess the byte code to store and fetch local vars would have to be 3
bytes (1 byte for the opcode and 2 bytes for the argument)
...
switch value expression. Then walk the cases. For each case, evaluate
its value expression. If the case value is equal to the switch value,
execute the statements under the case and then exit the ~switch~
statement. Otherwise, try the next case. If no case matches and there
is a ~default~ clause, execute its statements.
...
It’s a compile-time error to have a ~continue~ statement not enclosed
in a loop.

Make sure to think about scope. What should happen to local variables
declared inside the body of the loop or in blocks nested inside the
loop when a ~continue~ is executed?

** TODO Invent a useful novel control flow feature
...
For fun, try to invent a useful novel control flow feature for Lox. It
can be a refinement of an existing form or something entirely new. In
practice, it’s hard to come up with something useful enough at this
low expressiveness level to outweigh the cost of forcing a user to
learn an unfamiliar notation and behavior, but it’s a good chance to
practice your design skills.

* Calls and Functions
** TODO The ~ip~ in a native CPU register
Reading and writing the ~ip~ field is one of the most frequent
operations inside the bytecode loop. Right now, we access it through a
pointer to the current CallFrame. That requires a pointer indirection
which may force the CPU to bypass the cache and hit main memory. That
can be a real performance sink.
...
optimization. If we store the ~ip~ directly in a C local variable and
mark it ~register~, there’s a good chance the C compiler will accede
...
*** Answer
Add a local (register) var to contain the ip. Replace all occurences
of ~frame->ip~ with ~ip~. Set the ~ip~ in the ~OP_CALL~ and the
~OP_RETURN~ case. But I get a segfault.

See excercise/24.1 for the implementation.

** DONE Arity checking for native function
CLOSED: [2026-03-12 Do 10:49]
Native function calls are fast in part because we don’t validate that
the call passes as many arguments as the function expects. We really
...
*** Answer
Add an arity field to the ~ObjNative~ struct. Pass the arity when
defining native functions in ~defineNative~. Then check the arity in
the ~OBJ_NATIVE~ case in ~callValue~. Feels a bit clunky

See excercise/24.2 for the implementation.

** TODO Runtime errors for native functions
...
support because native functions live in the statically typed world of
C but are called from dynamically typed Lox land. If a user, say,
...
** DONE Add some more native functions
CLOSED: [2026-03-12 Do 10:49]
...
*** Answer
Add ~random~ and ~sleep~ (to have a function with an argument)

See excercise/24.4 for the implementation.
